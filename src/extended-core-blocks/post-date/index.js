...
/* global wp */
(function () {
    // Destructure the wp global here, so the function's dependencies are explicit.
    const {
        blocks: {
            getBlockVariations,
    ...
    domReady(() => {
        ...
        CORE_VARIATION_NAMES.forEach((variationName) => {
            ...
            try {
                ...
                console.error(
                    `Failed to unregister variation ${variationName}`,
                    e,
                );
            }
            ...
            const vDesc = isModified
                ? __(
                      "Displays the modified date prefixed with a label.",
                      "govwind",
                  )
                : __(
                      "Displays the post's date prefixed with a label.",
                      "govwind",
                  );
    ...
    const addPrefixControl = (BlockEdit) => (props) => {
        ...
        return el(
            ...
            el(
                ...
                el(BlockEdit, {
                    ...props,
                    __unstableDisableBlockWrapper: true,
                }),
            ),
        );
    };
