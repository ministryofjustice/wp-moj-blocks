...
function render_callback_featured_item_block($attributes, $content)
{
    ...
    // in edit(). It emits the generated wp-block-mojblocks-featured-item class,
    ...
    //
    // The is-style-* class has to land on the same element as
    // mojblocks-featured-item, because style.scss targets them together
    // (.mojblocks-featured-item.is-style-wide and similar).
    $featured_item_wrapper_attributes = get_block_wrapper_attributes(
        ['class' => 'mojblocks-featured-item']
    );
    ...
    <?php
        if ($attribute_box_featuredID != "" && get_post_type($attribute_box_featuredID) == $attribute_box_featuredType && get_post_status($attribute_box_featuredID) == "publish") {
            ...
            if (trim($attribute_box_customImage) != "") {
                $image = $attribute_box_customImage;
                $attribute_box_hasImage = true; // If a custom image has been set, the show image control is hidden and should always be considered true
    ...
    ?>
        <div <?php echo $featured_item_wrapper_attributes; ?>>
            <div class="govuk-width-container govuk-!-margin-0">
                ...
                <div class="mojblocks-featured-item__item">
                    <?php if ($image && $attribute_box_hasImage) { ?>
                        <div class="mojblocks-featured-item__image" style="background-image:url('<?php echo $image ?>')">
                            <span role="img" aria-label="Cover image for featured item (<?php _e(esc_html($feature_content["title"]));?>)"></span>
                        </div>
                    <?php } ?>
                    <div class="mojblocks-featured-item__text <?php if (!$attribute_box_hasBar) echo 'mojblocks-featured-item__text--no-bar';?>">
