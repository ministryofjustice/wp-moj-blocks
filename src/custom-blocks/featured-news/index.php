...
function render_callback_featured_news_block($attributes, $content)
{
    ...
    <?php
        if (get_post_type($attribute_box_featuredID) == "news") {
            ...
            // Wrapper attributes.
            //
            // get_block_wrapper_attributes() is the PHP counterpart to
            // useBlockProps() in edit(). It emits the generated
            // wp-block-mojblocks-featured-news class and the user's custom
            // classes, which WordPress persists in `className`.
            //
            // This previously read a featuredNewsClassName attribute that
            // nothing has ever written — not in current source and not in
            // history — so custom classes never reached the frontend for this
            // block. They will now.
            //
            // Built here rather than at the top of the callback because the
            // no-image modifier depends on $news_array.
            $featured_news_classes = ['mojblocks-featured-news'];

            if (!$news_array["image"]) {
                $featured_news_classes[] = 'mojblocks-featured-news--no-image';
            }

            $featured_news_wrapper_attributes = get_block_wrapper_attributes(
                ['class' => implode(' ', $featured_news_classes)]
            );
    ?>
        <div <?php echo $featured_news_wrapper_attributes; ?>>
