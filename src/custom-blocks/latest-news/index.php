...
function render_callback_latest_news_block($attributes, $content)
{
    ...
    // in edit(). It emits the generated wp-block-mojblocks-latest-news class and
    // the user's custom classes, which WordPress persists in `className`.
    //
    // This previously read a latestNewsClassName attribute that nothing has ever
    // written — not in current source and not in history — so custom classes
    // never reached the frontend for this block. They will now.
    $latest_news_wrapper_attributes = get_block_wrapper_attributes(
        ['class' => 'mojblocks-latest-news']
    );
    ...
    <div <?php echo $latest_news_wrapper_attributes; ?>>
