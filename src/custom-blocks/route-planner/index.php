...
function render_callback_route_planner_block($attributes, $content)
{
    ...
    ?>
        <form class="<?php _e(esc_html($attribute_class_name)); ?> mojblocks-route-planner">
            ...
            <button class="govuk-button" onclick="redirectToMap(event,'<?php echo esc_html($attribute_route_destination); ?>',<?php echo $rand_num;?>)"><?php _e("Submit","hale"); ?></button>
        </form>
        <script>
            if (typeof window.redirectToMap !== 'function') {
                function redirectToMap(e,dest,idPrefix) {
                    e.preventDefault();
                    const postcode = document.getElementById(idPrefix+"-postcodeInput").value.trim();
                    console.log(postcode);
                    if (!postcode) {
                        document.getElementById(idPrefix+"-postcodeInput-error").style.display = "block";
                        document.getElementById(idPrefix+"-postcodeInput").classList.add("govuk-input--error");
                        return;
                    }
                    document.getElementById(idPrefix+"-postcodeInput-error").style.display = "none";
                    document.getElementById(idPrefix+"-postcodeInput").classList.remove("govuk-input--error");

                    const origin = encodeURIComponent(postcode);
                    const destination = encodeURIComponent(dest); // your fixed location
